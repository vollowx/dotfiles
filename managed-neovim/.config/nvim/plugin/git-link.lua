...
local function copy_to_clipboard(text)
  vim.fn.setreg('+', text)
  vim.notify('[GitLink] URL copied to clipboard', vim.log.levels.INFO)
end
...
local function open_url_in_browser(url)
  ...
  elseif sysname == 'Linux' then
    ...
    vim.notify(
      '[GitLink] OS unsupported, unable to open URL',
      vim.log.levels.ERROR
    )
    return
  end
  vim.notify('[GitLink] Opening URL in browser', vim.log.levels.INFO)
  vim.fn.jobstart(cmd, { detach = true })
end

local function get_default_branch()
  local out =
    vim.fn.systemlist('git symbolic-ref refs/remotes/origin/HEAD 2>/dev/null')
  ...
  return vim.fn.trim(vim.fn.system('git rev-parse --short HEAD'))
end

local function get_current_branch()
  local branch =
    vim.fn.trim(vim.fn.system('git rev-parse --abbrev-ref HEAD 2>/dev/null'))
  if vim.v.shell_error ~= 0 or branch == '' or branch == 'HEAD' then
    vim.notify(
      '[GitLink] Could not determine branch, using default',
      vim.log.levels.WARN
    )
...
local function get_remote_url()
  ...
  local raw_url = vim.fn.trim(
    vim.fn.system(string.format('git config --get remote.%s.url', remote_name))
  )
  if vim.v.shell_error ~= 0 or raw_url == '' then
    vim.notify(
      ("[GitLink] No remote '%s' found"):format(remote_name),
      vim.log.levels.ERROR
    )
    return nil
  end
  ...
  vim.notify(
    '[GitLink] No URL rule matched for: ' .. raw_url,
    vim.log.levels.ERROR
  )
  return nil
end
...
local function get_url()
  ...
  if vim.v.shell_error ~= 0 then
    vim.notify('[GitLink] Not a git repository', vim.log.levels.ERROR)
    return nil
  end

  local cwd = vim.fn.getcwd():gsub('\\', '/')
  local abs_path = vim.fn.expand('%:p'):gsub('\\', '/')
  local rel_path = abs_path:gsub('^' .. vim.pesc(cwd) .. '/', '')

  local tracked = vim.fn.trim(
    vim.fn.system(
      'git ls-files --full-name -- ' .. vim.fn.shellescape(rel_path)
    )
  )
  if vim.v.shell_error ~= 0 or tracked == '' then
    vim.notify('[GitLink] File is not tracked by git', vim.log.levels.ERROR)
    return nil
  end
...
vim.api.nvim_create_user_command('GitLink', function(opts)
  ...
  elseif action == 'yank' then
    ...
    vim.notify('[GitLink] Usage: GitLink [open|yank]', vim.log.levels.WARN)
  end
end, {
  nargs = 1,
  range = true,
